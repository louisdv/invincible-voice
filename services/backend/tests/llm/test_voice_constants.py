...
def required_env(monkeypatch, tmp_path):
    """Set the env vars required for voice_constants to import without errors."""
...
def test_llm_url_is_none_when_env_var_empty(required_env):
    ...
    importlib.reload(voice_constants)
    assert voice_constants.LLM_URL is None
...
def test_llm_api_key_is_none_when_env_var_empty(required_env):
    ...
    importlib.reload(voice_constants)
    assert voice_constants.LLM_API_KEY is None
